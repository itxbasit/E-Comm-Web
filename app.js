...
function createPagination(totalPage, skip) {
  let paginationHTML = `<li onclick="console.log(${skip})">
  ...
  for (let i = 1; i <= totalPage; i++) {
    paginationHTML += `<li onclick="fetchApi(${(i - 1) * 15})">
        <a  class="flex items   -center justify-center text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading font-medium text-sm w-9 h-9 focus:outline-none">${i}</a>
      </li>`;
  }

  paginationHTML += `<li>
      <a  class="flex items-center justify-center text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading font-medium rounded-e-base text-sm w-9 h-9 focus:outline-none">
...
function fetchProducts(data) {
  let totalPage = Math.ceil(data.total / data.limit);
  createPagination(totalPage, data.skip);
  list.innerHTML = "";
  data.products.forEach((product) => {
    list.innerHTML += `<div onclick="window.location.href='detail.html?productId=${product.id}'"
